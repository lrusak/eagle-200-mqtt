                ...
                while True:
                    try:
                        async with aiomqtt.Client(
                            hostname=self.mqtt_ip, port=self.mqtt_port
                        ) as client:
                            ...
                            while True:
                                ...
                                if len(query) > 0 and len(query["Components"]) > 0:
                                    ...
                                    await client.publish(
                                        "power/home",
                                        f"power,location=home,sensor=eagle-200 value={power}",
                                    )
                        ...
                        print(
                            f"Connection lost; Reconnecting in {self.interval} seconds ..."
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Send power information from an eagle-200 to an MQTT broker",
    ...
    optional.add_argument(
        "--interval", dest="interval", type=int, default=5, help="Interval in seconds"
    )
